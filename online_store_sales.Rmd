...
In this project I am developed a **time series forecasting** model of $h$ = **28** forecasting horizon.
...
The time-series dataset was of a **multi-variate** nature which we then reduced into a **univariate** time-series in order to focus our analysis on a specific subset of the data.

The dataset contained a number of inconsistencies that needed to be normalised before commencing any the analysis, for example missing values, outliers, non-standardised product codes etc.
For that reason, a significant part of this project has been devoted in **exploratory data analysis** and in **time-series analysis**.
Due to the large number of independent time series in the dataset (over three thousand product codes), I decided to group teh data in **higher granularity** product categories based on a **two dimensional 80/20** rule. The new product hierarchy consisted of:
...
I have combined the above categories into a **two by two table** to review all combinations and isolated the products belonging in the **"AH"** intersection i.e., products of **high revenue** and **large volume of orders** which have the majority of impact on both the company's top and bottom lines.
The time-series analysis helped to identify the **trend** and weekly **seasonality** pattern in the data.
I have used those features to fine tune the algorithms used during the development of our model.
Specifically, I've identified that there is an **upward additive trend** in the series and that sales tend to peak around the **middle of each week**.

The methodology commenced by splitting our data into a **28 days** long dataset named **online_store_validate** which was reserved until the very end of the analysis to test the model's accuracy, and a fit dataset named **online_store_fit** which contains all data prior to the validation set.
I have have further proceeded in splitting the online_store_fit data into **train and test** sets which where used to develop the model.
...
I have used the **Mean Absolute Percentage Error (MAPE)** as the loss function. Percentage errors have the advantage of being unit-free, and so are frequently used to compare forecast performances between data sets [@hyndman_2021].

I then proceeded with a mathematical overview and application of the forecasting methods and their corresponding algorithms as well as examining the score results of each algorithm's performance over the test dataset.
I provided a **visual representation** of the forecasts generated from the train set's **fitted values** as well as their **80% and 95% confidence intervals**.
...
To ensure I haven't over fitted the model and that is generalised enough to produce unbiased forecasts based on new data, I have used **time series bootstrapping** to simulate **100 time series** and produced forecasts using the **Exponential Smoothing** algorithm [@BERGMEIR2016303].
I then averaged the individual forecasts ("forecast bagging") and measured the accuracy against the **validation dataset**.
I have observed a **marginal accuracy improvement** with a **MAPE = 24%** after this process.
...
In this early stage of the exploratory analysis, I have used the [skimr](https://cran.r-project.org/web/packages/skimr/vignettes/skimr.html) package as way to quickly review different statistics about the dataset.
...
Based on Skimr's output we can see that our dataset consists of 541,909 rows and 8 columns in total and of types character, numeric and timestamp.
...
The latter is missing a significant percentage of it's values, so at this stage I am flagging it as a candidate to be ignored in our further analysis.
...
These are potential outliers that I later identify and either remove or replace to avoid skewing the results of the analysis.
Finally, the time-stamp range is approximately a year with the earliest sale recorded in December 2010 and the latest one in December 2011.
...
I examined the bottom 10 prices in the dataset.
...
There's also a number of products with zero prices, meaning they have contributed to the company's revenue.
...
Another important observation is that product codes (StockCode) appear to differentiate a lot, both in their formats and character length.
...
I first looked at the shorter length code and examine what products do they refer to.
...
Similarly, if I filter on the top 10 products based on Quantity.
We can identify the large value observed earlier.
The first and second observations, although they refer to actual products and have contributed to the company's revenue (price \>0), they appear to have been one-off.
...
At this point I started cleaning up some of these inconsistencies and reviewed again what our data look like.
Specifically I have filtered out any entries with prices less than zero.
...
I am interested in forecasting the volume of stock that the business will need to deliver to customers so negative quantities need to be factored out.
Additionally, I only kept entries whose product codes are of character length 5 or 6.
That way the dataset consists of actual products.
...
The new skimr output is a lot cleaner and something I can move into the next phase of the analysis.
...
A final check I need to do with regards to product codes is to check if we'd left in any with a character length 5 but of non-numeric type as the actual product codes are.
