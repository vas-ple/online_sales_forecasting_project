...
The focus of this project is to develop a **time series forecasting** model of $h$ = **28** daily forecasting horizon of an on-line UK retailer.
The dataset analysed was an extract from the online store's sales database located in the [UCI Machine Learning Repository](https://archive.ics.uci.edu/ml/datasets/online+retail) .
...
Due to the large number of independent time series in the dataset (over three thousand product codes), and to improve the quality of the forecast the data was grouped in **higher granularity** product categories based on a **two dimensional 80/20** rule. The new product hierarchy consisted of:
...
The methodology commenced by splitting our data into a **28 days** long dataset named **online_store_validate** which was reserved until the very end of our analysis to test our model's accuracy, and a fit dataset named **online_store_fit** which contains all data prior to the validation set.
...
The **Mean Absolute Percentage Error (MAPE)** was used as our loss function. Percentage errors have the advantage of being unit-free, and so are frequently used to compare forecast performances between data sets [@hyndman_2021].
...
Based on Skimr's output we can see that our dataset consists of 541,909 rows and 8 columns in total and of types character, numeric and time-stamp.
...
The logic is that is non numeric product codes exist, the coercion will introduce NA which will then isolate and review.
...
UK sales dominate our market spread, which is natural since our company is a UK on-line store.
...
There is a small number of products (AL) that also contribute to the 80% revenue baseline but they don't have a large number of orders against them.
We can assume that these are high price point items or potentially infrequent bulk orders of items.

Perhaps from a business perspective the items that will need to reviewed by management are the ones belonging to the BH category.
...
In this analysis, we developed a time series forecasting model of $h$ = 28 time horizon.
